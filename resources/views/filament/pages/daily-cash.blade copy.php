<x-filament-panels::page>
    <div class="space-y-4" x-data="{
        tipoFiltro: @entangle('tipoFiltro'),
        fecha: @entangle('fecha'),
        fechaInicio: @entangle('fechaInicio'),
            ...
            <div class="flex flex-wrap gap-4">
                ...
                <div class="flex-1 min-w-[200px]">
                    <template x-if="tipoFiltro === 'fecha'">
                        <div>
                            ...
                            <input type="date" x-model="fecha" class="block w-full border-gray-300 rounded-lg shadow-sm">
                        </div>
                    </template>
                    <template x-if="tipoFiltro === 'rango'">
                        <div>
                            ...
                            <div class="flex gap-2">
                                <input type="date" x-model="fechaInicio" class="w-full border-gray-300 rounded-lg shadow-sm">
                                <input type="date" x-model="fechaFin" class="w-full border-gray-300 rounded-lg shadow-sm">
                            </div>
                        </div>
                    </template>
                </div>
                ...
                <div class="flex items-end">
                    <button
                        ...
                        class="px-4 py-2 text-sm font-medium text-white rounded-lg bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
                    >
                        Actualizar
                    </button>
                </div>
        ...
        <div class="flex flex-wrap gap-4">
            ...
            <div class="flex-1 min-w-[250px] bg-white rounded-lg shadow">
                ...
                <div class="p-3 space-y-2">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm text-primary-600">${{ number_format($ventasData['facturas']['FacturasGalpon'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">Otros:</span>
                        <span class="text-sm text-primary-600">${{ number_format($ventasData['facturas']['FacturasOtros'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">Nulas:</span>
                        <span class="text-sm font-medium text-red-600">{{ $resumen['general']['cantidades']['facturasAnuladas'] ?? 0 }}</span>
                    </div>
            ...
            <div class="flex-1 min-w-[250px] bg-white rounded-lg shadow">
                ...
                <div class="p-3 space-y-2">
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">Galpón:</span>
                        <span class="text-sm text-primary-600">${{ number_format($ventasData['boletas']['BoletasGalpon'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">Otros:</span>
                        <span class="text-sm text-primary-600">${{ number_format($ventasData['boletas']['BoletasOtros'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">Nulas:</span>
                        <span class="text-sm font-medium text-red-600">{{ $resumen['general']['cantidades']['boletasAnuladas'] ?? 0 }}</span>
                    </div>
                    <div class="flex items-center justify-between pt-2 border-t">
                        <span class="text-xs text-gray-500">Promedio:</span>
            ...
            <div class="flex-1 min-w-[250px] bg-white rounded-lg shadow">
                <div class="flex items-center justify-between p-2 border-b">
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 text-xs font-medium text-orange-700 bg-orange-100 rounded-full">
                        {{ count($ventasData['srf']['clientes'] ?? []) }} clientes
                    </span>
                </div>
                <div class="p-3 space-y-2">
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">Total:</span>
                        <span class="text-lg font-bold text-primary-600">${{ number_format($ventasData['srf']['totalGeneral'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    @foreach($ventasData['srf']['clientes'] ?? [] as $cliente)
                        <div class="flex items-center justify-between">
            ...
            <div class="flex-1 min-w-[250px] bg-white rounded-lg shadow">
                ...
                <div class="p-3 space-y-2">
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">Galpón:</span>
                        <span class="text-sm text-primary-600">${{ number_format($ventasData['totales']['montoGalpon'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">Otros:</span>
                        <span class="text-sm text-primary-600">${{ number_format($ventasData['totales']['montoOtros'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">SRF:</span>
                        <span class="text-sm text-primary-600">${{ number_format($ventasData['srf']['totalGeneral'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">Nulas:</span>
                        <span class="text-sm font-medium text-red-600">{{ $resumen['general']['cantidades']['anulados'] ?? 0 }}</span>
                    </div>
                    <div class="flex items-center justify-between pt-2 border-t">
                        <span class="text-xs font-medium text-gray-500">Total + SRF:</span>
                        <span class="text-lg font-bold text-green-600">${{ number_format(($ventasData['totales']['montoTotal'] ?? 0) + ($ventasData['srf']['totalGeneral'] ?? 0), 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
        </div>
